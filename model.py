...
class myModel(nn.Module):
    ...
    def _key_addressing(self, h_emb_list, r_emb_list, t_emb_list, technique_embeddings):
        ...
        for hop in range(self.n_hop):
            ...
            # [batch_size, n_memory]
            try:
                probs_normalized = torch.sigmoid(probs)
            except:
    ...
    def predict(self, cm, demand_enterprise, cm_num, enterprise_num, demand_num, cm_ripple_set, enterprise_ripple_set):
        ...
        cbe_matrix = F.softmax(torch.mm(self.W_u1(cm_emb), self.W_v1(demand_emb).T))
        ebc_matrix = F.softmax(torch.mm(self.W_v2(enterprise_emb), self.W_u2(cm_emb).T))
